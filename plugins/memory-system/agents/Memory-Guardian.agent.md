...
description: Capture, compress, and retrieve agent session memory. Manages session-memory.md, decision-log.md, and the mcp-backed knowledge graph.
...
- **Level 3 (MCP knowledge graph):** `.vscode/mcp.json` is present and configures `@modelcontextprotocol/server-memory`. This plugin documents MCP-backed behavior, but the current editor validator does not allow an `mcp` frontmatter tool declaration here. Use Level 3 operations only when the host runtime exposes MCP access; otherwise operate in degraded file-backed mode.
- When the MCP server is unreachable at runtime, fall back to Level 1 and Level 2 files and note the degraded mode.
